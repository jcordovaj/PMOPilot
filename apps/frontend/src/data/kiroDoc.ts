export const KIRO_TRANSFER_CONTEXT_MD = [
  ...
  '**Stack del Backend (Kiro)**: **Python 3.11+ (FastAPI + Pydantic)** + **Amazon DynamoDB (boto3)**.',
  'Debes aprovisionar las tablas o esquema Single-Table en **Amazon DynamoDB** (utilizando el SDK oficial de Python `boto3` o `pydantic-settings`) con la estructura de atributos definida a continuación para garantizar una arquitectura nativa en AWS sin duplicidad de dominios ni bases de datos secundarias:',
  '',
  ...
  '- **Frontend envía**: Datos del repositorio Git (repo_url, github_token, branch), servicios de mensajería (sendgrid_key, sender_email), gestión (jira_domain, token), red (cloudflare_zone) y AWS (region, dynamodb_prefix).',
  '- **Tu tarea**: Almacenar la configuración en DynamoDB (ítem "PROJECT#CONFIG"), configurar secretos en tiempo de ejecución e inicializar el receptor de webhooks de GitHub Actions ("/api/webhooks/github").',
  '',
  '---',
  '',
