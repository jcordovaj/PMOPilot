...
PmoPilot es un **Cockpit de Orquestación Semántica** que actúa como asistente inteligente y PMO para equipos de desarrollo sin conocimiento técnico, que siguen la metodología **SDD (Specification-Driven Development)** para implementar sus ideas. Permite a fundadores, emprendedores, y líderes de ingeniería gobernar el ciclo de vida de un proyecto de software mediante intenciones y objetivos en lenguaje natural, manteniendo alineados en tiempo real los documentos de arquitectura (System Specifications, ADRs), el tablero Kanban, planeación de tareas, las auditorías de Pull Requests y la infraestructura perimetral. Es una capa de abstracción sobre los generadores de código con IA, que elimina la complejidad de conocer lenguajes y herramientas, necesarios para que un proyecto se implemente de principio a fin.
...
2. **Colaborador / Tester / Ingeniero de QA (`tester`)**: Representada por **Ana**. Puede abrir incidencias, auditar tareas de ingeniería y proponer PRs de regresión, pero tiene bloqueado el acceso de escritura a la infraestructura de red. Puede proponer funcionalidades o cambios de alcance, crear epics, especificaciones, las que llegan al líder como PRs.

3. **Invitado (`guest`)**: Representado por **David** (u otros colaboradores externos). Cuenta con acceso de solo lectura en las consolas de observabilidad e historial del proyecto. Puede hacer comentarios, recibir notificaciones, participar del chat de mensajería, pero no puede modificar artefactos.

---

## 3. Arquitectura de Datos y Entidades en Amazon DynamoDB

**Stack Tecnológico del Backend (Kiro)**: Python 3.11+ (FastAPI + Pydantic + Uvicorn) + Amazon DynamoDB (boto3 / pynamodb).

Todo el desarrollo se hará con DynamoDB en forma local, que permitirá probar los modelos y no modificar lógica al pasar del ambiente local a la nube en AWS.
...
Para el desarrollo local debes aprovisionar las tablas o diseño Single-Table en **Amazon DynamoDB** (utilizando `boto3` / `pydantic_settings` en Python) con la estructura de atributos definida a continuación para garantizar una arquitectura nativa en AWS sin duplicidad de dominios ni bases de datos secundarias:
...
- **Tu tarea**: Usar Gemini con Function Calling o prompts estructurados para diagnosticar si el intent de la instrucción de lenguaje natural del usuario implica una mutación del estado del proyecto. El usuario no es técnico, gemini debe clarificar cualquier requerimiento. Debe retornar una respuesta conversacional fluida recomendando objetos opcionales `proposedAction` o `logic next steps` (ej: `CREATE_TASK`, `ADD_MEMBER`, `UPDATE_TASK_STATUS`, `BOOTSTRAP_PROJECT`) con los datos ya pre-estructurados por la IA para que el cliente los aplique con un solo clic.
